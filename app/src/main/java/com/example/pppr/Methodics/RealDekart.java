...
import com.example.pppr.databinding.ActivityRealdekartBinding;

import java.util.List;

public class RealDekart extends AppCompatActivity {
    private ActivityRealdekartBinding binding;
    private int k = 0;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        binding = ActivityRealdekartBinding.inflate(getLayoutInflater());
        setContentView(binding.getRoot());
        String savedtext = getIntent().getStringExtra("savedText");

        final EditText input = new EditText(RealDekart.this);

        initDatabase();
        binding.textView.setText(savedtext);

        AlertDialog.Builder builder = new AlertDialog.Builder(RealDekart.this);
        builder.setMessage("")
                ...
                .setPositiveButton("Ок", new DialogInterface.OnClickListener() {
                    public void onClick(DialogInterface dialog, int whichButton) {
                        ...
                        if (k == 1){
                            ...
                            ContentDB content = new ContentDB(savedtext, 4, 1, userInput);
                            ...
                            System.out.println(savedtext + "      " + userInput);
                            ContentDB content = new ContentDB(savedtext, 4, 2, userInput);
                            database.getContentDao().insert(content);
                            adapter2.addNewPublication(content);
                            input.setText("");
                        }
                        else if (k == 3){
                            ContentDB content = new ContentDB(savedtext, 4, 3, userInput);
                            database.getContentDao().insert(content);
                            adapter3.addNewPublication(content);
                            input.setText("");
                        }
                        else if (k == 4){
                            ContentDB content = new ContentDB(savedtext, 4, 4, userInput);
                            database.getContentDao().insert(content);
    ...
    private void initRecyclerView(String savedtext) {
        List<ContentDB> contentDBList1 = database.getContentDao().selectcontent(savedtext, 4, 1);
        List<ContentDB> contentDBList2 = database.getContentDao().selectcontent(savedtext, 4, 2);
        List<ContentDB> contentDBList3 = database.getContentDao().selectcontent(savedtext, 4, 3);
        List<ContentDB> contentDBList4 = database.getContentDao().selectcontent(savedtext, 4, 4);
